...
M=squeeze(mean(Img,[1,2])); % mean intensity of each frame
ImgMax = max(Img, [], 3);
%ImIF = I- IF;
warning('off');
...
if NeedCorrect == 1
    ...
    BWBlood=ImgMax<mean(ImgMax,'all')*0.8 & ImgMax<IM*0.7;
...
if any(AContinue == 'y')
    ...
    for i=[1:10:40,41:3:200 201:10:size(Img,3)]
        ...
        BW = I>IF*Thresh & I>IM*1.1 & I>M(i)*0.9;
        BW(1:10,:)=0;
        BW(:,1:10)=0;
        BW(end-10:end,:)=0;
        BW(:,end-10:end)=0;
