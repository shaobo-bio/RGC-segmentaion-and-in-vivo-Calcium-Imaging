function [] = sbc2_Analysis(sigsToMeasure, rawSigsToMeasure, tvals, cellIDs, approxStart, approxEnd, viewPlots, saveFlag, saveDir)
...
figure; histogram(sigWidthCleaned,'binwidth',1);
    xlabel('Time (sec)');
    ylabel('Frequency');
    title('ON-Sustained 2 Signal Width');
    set(gca,'FontSize',14,'fontweight','bold');
%%
fid = fopen([saveDir '\results.txt'],'w');
fprintf(fid,'Cell Type: OFF-Suppression\n');
...
T = num2cell([tsRiseCleaned; tsFallCleaned; sigWidthCleaned]);
T = table([cellIDsCleaned; [{'tRise (sec)' 'tFall (sec)' 'sigWidth (sec)'}' T]]);
