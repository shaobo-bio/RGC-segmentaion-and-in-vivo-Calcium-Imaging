function [] = on_sust3_Analysis(sigsToMeasure, rawSigsToMeasure, tvals, cellIDs, approxStart, approxEnd, viewPlots, saveFlag, saveDir)
...
for pp = 1:size(sigsToMeasure,1)
    ...
    if ~isempty(dfPk) && ~isempty(dfPk2)
        ...
        sigAmp1(pp) = mean(tmpData(zx(startInd+1)-round(ampAvgWidth/2):zx(startInd+1)+round(ampAvgWidth/2)));
        ...
        sigAmp2(pp) = mean(tmpData(zx(startInd+2)-round(ampAvgWidth/2):zx(startInd+2)+round(ampAvgWidth/2)));
...
onSust3Cleaned = sigsToMeasure;
onSust3CleanedRaw = rawSigsToMeasure;
ampsCleaned = sigAmp1-sigAmp2;
...
onSust3Cleaned(riseIntDiff<=0,:) = [];
onSust3CleanedRaw(riseIntDiff<=0,:) = [];
...
onSust3Cleaned(fallIntDiff<=0,:) = [];
onSust3CleanedRaw(fallIntDiff<=0,:) = [];
...
onSust3Cleaned(tsFallCleaned<=0,:) = [];
onSust3CleanedRaw(tsFallCleaned<=0) = [];
...
%     xlim([10 30]);
    xlabel('Time (sec)');
    ylabel('Frequency');
    title('Signal Width - ON-Sustained 3');
    set(gca,'fontweight','bold','FontSize',14);
    if saveFlag
...
figure; histogram(ampsCleaned); 
    xlabel('Amplitude'); 
    ylabel('Frequency'); 
    title('Signal Amplitudes - ON-Sustained 3');
    set(gca,'fontweight','bold','FontSize',14);
    if saveFlag
...
fprintf(fid,'Cell Type: ON-Sustained 3\n');
fprintf(fid,'N = %d\n', length(onSust3Cleaned));
...
T = num2cell([tvals'; onSust3CleanedRaw]');
